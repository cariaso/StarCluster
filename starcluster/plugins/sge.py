...
class SGEPlugin(clustersetup.DefaultClusterSetup):
    ...
    def _set_sge_hvmem_node(self, node):
        ...
        node.ssh.execute(
            'qconf -rattr exechost complex_values h_vmem=%sG'
            ',mem_free=%sG,slots=%s %s' %
    ...
    def _setup_sge(self):
        ...
        master.ssh.execute(
            "qconf -sc | sed '/h_vmem              h_vmem     "
            "MEMORY      <=    YES         NO         0       "
            "0/ c\h_vmem              h_vmem     MEMORY      <="
            "YES         JOB        0        0' > sge.conf")
        master.ssh.execute('qconf -Mc sge.conf && rm sge.conf')
        master.ssh.execute(
            "qconf -sc | sed '/mem_free              mem_free     "
            "MEMORY      <=    YES         NO         0        0/ "
            "c\mem_free              mem_free     MEMORY      <=  "
            "YES         YES        0         0' > sge.conf")
        master.ssh.execute('qconf -Mc sge.conf && rm sge.conf')
